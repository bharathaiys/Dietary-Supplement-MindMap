...
transformGraphToMindmap = function(graph) {
    ...
    graph.forEach(pair => {
        if (!nodes[pair.nodes[0].labels[0]]) {
            nodes[pair.nodes[0].labels[0]] = new Set();;
        }
        nodes[pair.nodes[0].labels[0]].add(pair.nodes[0]);

        if (!nodes[pair.nodes[1].labels[0]]) {
            nodes[pair.nodes[1].labels[0]] = new Set();;
        }
        nodes[pair.nodes[1].labels[0]].add(pair.nodes[1]);
    
        
        if (!relationships[pair.relationships[0].type]) {
            relationships[pair.relationships[0].type] = new Set();;
        }
        relationships[pair.relationships[0].type].add(pair.relationships[0]);
    });
    console.log(nodes);
