...
transformGraphToMindmap = function(graph) {
    ...
    let relationship = {};
    graph.forEach(pair => {
        if (root[pair.nodes[0].labels[0]]) {
            root[pair.nodes[0].labels[0]].push(pair.nodes[0]);
        } else {
            root[pair.nodes[0].labels[0]] = [];
        }

        if (root[pair.nodes[1].labels[0]]) {
            root[pair.nodes[1].labels[0]].push(pair.nodes[1]);
        } else {
            root[pair.nodes[1].labels[0]] = [];
        }
        
        if (relationship[pair.relationships[0].type]) {
            relationship[pair.relationships[0].type].push(pair.relationships[0]);
        } else {
            relationship[pair.relationships[0].type] = [];
        }
