...
export class MindmapComponent implements OnInit {
  ...
  private _filterds(value: string): any[] {
    this.sympton = value;
    const filterValue = value.toLowerCase();
    if (value == '') {
      return [];
  ...
  constructor(private _snackBar: MatSnackBar, private route: ActivatedRoute) {
    this.ingredCtrl.setValue("Vitamin D");
    ...
    this.filteredingredient = this.ingredCtrl.valueChanges
      ...
      this.dsCtrl.setValue("Asthmas");
      console.log(this.dsCtrl);
      this.filteredds = this.dsCtrl.valueChanges
        .pipe(
          startWith(''),
          map(ds => ds ? this._filterds(ds) : this.ds_slice(ds))
        );
  }
  durationInSeconds = 5;
  openSnackBar() {
  ...
  queryServer = function () {
    ...
    if (this.problemId === '1') {
      data = {
        query: `Match (a:DSP)-[b]-(c:SDSI {name:'${this.selectedIngredient}'}) return a, b, c limit 10;`
      }
      q_type = '1';
    } else if (this.problemId === '2') {
      data = {
        query: `Match (a:DSP)-[:has_ingredient]-(SDSI)-[b:is_effective_for]-(c:DIS {name:'${this.sympton}'}) return a, b, c limit 10;`
      }
      q_type = 'production';
    }
    else if (this.problemId === '3') {
      data = {
        query: `Match (a:SDSI)-[b:is_effective_for]-(c:DIS {name:'${this.sympton}'}) return a, b, c limit 10;`
      }
      q_type = 'ingredient';
    ...
    if (q_type === "ingredient") {
      ...
      for (var key in nodes['DSP']) {
        ...
        let tempNode = {
          ...
          id: nodes['DSP'][key].id,
          type: 'DSP',
          background: nodes['DSP'][key].properties.background
        };
      ...
      for (var key in nodes['SS']) {
        ...
        let tempNode = {
          name: nodes['S'][key].properties.name,
          id: nodes['DSP'][key].id,
          type: 'DSP',
          background: nodes['DSP'][key].properties.background
        };
  ...
  update(source) {
    ...
    linkUpdate
      ...
      .style("stroke", function(d) {
        let parentType = d.parent.data.type;
        ...
        if (parentType === "DSP" && type === "DIS") {
          return "url(#r-is-effective-for)";
        } else if (type === "DSP" && parentType === "DIS") {
          return "url(#is-effective-for)";
        }

        if (parentType === "DSP") {
          return "url(#has-ingredient)"
        } else if (type === "DSP") {
          return "url(#r-has-ingredient)"
        }

        if (parentType === "PD") {
          return "url(#interacts-with)"
        } else if (type === "PD") {
          return "url(#r-interacts-with)"
        }
        ...
        if (parentType === "SS") {
          return "url(#has-adverse-reaction)"
        } else if (type === "SS") {
          return "url(#r-has-adverse-reaction)"
        }

      }).
      attr("id", function(d) {
        return "linkid_"+d.data.id;
      });
  ...
  findMore(d, type) {
    ...
    fetch("http://54.196.96.108:5001/api/v1/query", {
      ...
      d3.selectAll("path.link").classed("path-gray", true);
      d3.selectAll("text").classed("gray", true);

      selectedNode.children.forEach((el) => {
        console.log(el);
        d3.select("#id_" + el.id).select(".node").classed("gray", false);
        d3.select("#id_" + el.id).select("text").classed("gray", false);
        d3.select("#linkid_" + el.id).classed("path-gray", false);
      });

      let parent = d;
      while (parent) {
        d3.select("#id_" + parent.data.id).select(".node").classed("gray", false);
        d3.select("#id_" + parent.data.id).select("text").classed("gray", false);
        d3.select("#linkid_" + parent.data.id).classed("path-gray", false);
