...
export class MindmapComponent implements OnInit {
    ...
    nodeEnter.append('circle')
      ...
      .style("fill", function (d: any) {
        ...
        if (d.data.type == "DIS") {
          ...
          return d._children ? "lightsteelblue" : "rgba(197, 82, 226)";
    ...
    nodeUpdate.select('circle.node')
      ...
      .style("fill", function (d: any) {
        if (d.data.type == "DIS") {
          ...
          return d._children ? "lightsteelblue" : "rgba(197, 82, 226, 0.667)";
        } else if (d.data.type == "TC") {
          return d._children ? "lightsteelblue" : "#363";
        } else if (d.data.type == "SOC") {
          return d._children ? "lightsteelblue" : "#a739";
  ...
  collapse(d) {
    ...
    if (d.children || d._children) {
      ...
      d3.select("#id_" + d.data.id).attr("x", function (d: any) {
        console.log(d);
        return d.children ? -13 : 13;
      })
      // if (d3.event.defaultPrevented) {
  ...
  findMore(d) {
    ...
    fetch("http://54.196.96.108:5001/api/v1/query", {
      ...
      for (let nodes of res.graph) {
        ...
        if (!this.nodeHash[nodes.nodes[0].id]) {
          let tempNode = {
        ...
        if (!this.nodeHash[nodes.nodes[1].id]) {
          let tempNode = {
