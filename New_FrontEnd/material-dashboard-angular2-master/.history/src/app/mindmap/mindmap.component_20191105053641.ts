...
export class MindmapComponent implements OnInit {
  ...
  transformGraphToMindmap = function (graph, q_type) {
    ...
    graph.forEach(pair => {
      ...
      nodes[pair.nodes[0].labels[0]][pair.nodes[0].data.id] = pair.nodes[0];

      if (!nodes[pair.nodes[1].labels[0]]) {
        nodes[pair.nodes[1].labels[0]] = {};
      }
      nodes[pair.nodes[1].labels[0]][pair.nodes[1].data.id] = pair.nodes[1];

      if (!relationships[pair.relationships[0].type]) {
        relationships[pair.relationships[0].type] = [];
      }
      relationships[pair.relationships[0].type][pair.relationships[0].data.id] = pair.relationships[0];
    });
    console.log(nodes);
