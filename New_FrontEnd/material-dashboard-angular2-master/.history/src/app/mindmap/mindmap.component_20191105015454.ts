...
export class MindmapComponent implements OnInit {
  ...
  zoom = d3.behavior.zoom().scale(this.scale)
  ...
  container = this.vis.append("svg:g")
    .attr("id", "container")
  ...
  update(source, slow) {
    ...
    var nodesLeft = this.tree
      .size([this.h, (this.w / 2) - 20])
      .children(function (d) {
        return (d.depth === 0) ? d.left : d.children;
      })
      .nodes(this.root)
      .reverse();
    var nodesRight = this.tree
      .size([this.h, this.w / 2])
      .children(function (d) {
    ...
    var nodes = this.toArray(this.root);
    ...
    this.bbox = this.container.node().getBBox();
    ...
    */
    ...
    var link = this.container.selectAll("path.link")
      .data(this.tree.links(nodes), function (d: any) {
        return d.target.id;
      });
