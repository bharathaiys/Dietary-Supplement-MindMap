  ...
  findMore(d, type) {
    ...
    fetch("http://54.196.96.108:5001/api/v1/query", {
      ...
      for (let nodes of res.graph) {
        ...
        if (!this.nodeHash[nodes.nodes[0].id] && !this.atomNode.includes(nodes.nodes[0].type)) { 
          let tempNode = {
        ...
        if (!this.nodeHash[nodes.nodes[1].id] !this.atomNode.includes(nodes.nodes[0].type)) {
          let tempNode = {
