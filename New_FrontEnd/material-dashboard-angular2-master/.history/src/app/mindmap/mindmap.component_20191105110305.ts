...
function update(source) {
  ...
  function getTransform(node, xScale) {
    ...
    console.log(bbox);
    var bx = d3.zoomTransform(node.attr("transform")).translate[0];
    var by = d3.zoomTransform(node.attr("transform")).translate[1];
    var bw = bbox.width;
    var bh = bbox.height;
    var tx =  this.vw / 2 - bx * xScale + bw * xScale / 2;
    var ty = t+ this.vh / 2 - by * xScale - bh * xScale / 2;
    console.log(bx);
    console.log(by);
    return {
      translate: [tx, ty],
